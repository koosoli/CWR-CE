...
// ISAAC cryptographic RNG (Jenkins), as a C++ template parameterised on ALPHA.
// Expected period for ALPHA=8, 32-bit words is roughly 2^8295.
...
template <int ALPHA, class T>
QTIsaac<ALPHA, T>::~QTIsaac(void) = default;
...
{
    return (!m_rc.randcnt-- ? (isaac(&m_rc), m_rc.randcnt = (N - 1), m_rc.randrsl[m_rc.randcnt])
